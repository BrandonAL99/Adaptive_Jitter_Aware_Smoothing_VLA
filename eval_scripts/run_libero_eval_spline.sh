#!/bin/bash
# AJASSpline eval across all 4 LIBERO suites — laptop/CPU.
#
...
#
# Expected runtime: ~2-4 min/ep on laptop CPU → ~13-27 hrs total
# Tip: run libero_object first (~3-7 hrs) to get a headline number,
#      then continue with remaining suites.
# Results are local only (--no-push). Run GPU script for HF upload.
#
# Usage (from repo root):
#   conda activate lerobot
#   bash run_libero_eval_spline.sh
...
LOG_DIR="results/logs/libero_eval_spline"
OUT_BASE="results/eval_spline"
...
echo " Total eps  : $TOTAL_EPS  (~$(( TOTAL_EPS * 120 / 3600 )) hrs at ~2 min/ep on CPU)"
echo " Exec steps : $EXEC_STEPS"
echo " Seed       : $SEED"
echo " C          : $SPLINE_C"
echo " k_min      : $SPLINE_K_MIN   k_max=$SPLINE_K_MAX"
echo " sm_window  : $SPLINE_SM_WIN"
echo " kf_window  : $SPLINE_KF_WIN"
...
for SUITE in "${SUITES[@]}"; do
    HF_DATASET="BrandonAL/eval_smolvla_spline_c${SPLINE_C}_${SUITE}"
    OUT_DIR="${OUT_BASE}/${SUITE}"
    LOG_FILE="${LOG_DIR}/${SUITE}.log"

    # Skip suite only if all tasks completed (overall key present)
    ...
    if [ -f "${OUT_DIR}/eval_info.json" ]; then
        SR=$(python3 -c "
import json; d=json.load(open('${OUT_DIR}/eval_info.json'))
v=d.get('overall',{}).get('success_rate')
print(f'{v:.1%}' if isinstance(v,float) else 'N/A')")
        SM_RAW=$(python3 -c "
import json; d=json.load(open('${OUT_DIR}/eval_info.json'))
v=d.get('overall',{}).get('tc_raw',{}).get('sm_mean')
print(f'{v:.5f}' if isinstance(v,float) else 'N/A')")
        SM_PROC=$(python3 -c "
import json; d=json.load(open('${OUT_DIR}/eval_info.json'))
v=d.get('overall',{}).get('tc_proc',{}).get('sm_mean')
print(f'{v:.5f}' if isinstance(v,float) else 'N/A')")
        echo ""
...
ser     = sm_proc / sm_raw if (sm_raw and sm_proc) else None
print(f' {\"$SUITE\":<18}  {(str(round(sr*100,1))+\"%\") if isinstance(sr,float) else \"N/A\":>7}  '
      f'{sm_raw:.5f}  {sm_proc:.5f}  {ser:.4f}' if all(x is not None for x in [sr,sm_raw,sm_proc,ser])
      else f' {\"$SUITE\":<18}  N/A')
"
    else
        printf " %-18s  %7s\n" "$SUITE" "MISSING"
